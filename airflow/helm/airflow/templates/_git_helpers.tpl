...
{{- else }}
  args:
    ...
    {{- if .context.Values.git.dags.enabled }}
      ...
      is_mounted_dir_empty "/dags_{{ include "airflow.git.repository.name" . }}" && git -c http.sslVerify=false clone {{ .repository }} --branch {{ .branch }} /dags_{{ include "airflow.git.repository.name" . }}
      {{- end }}
    {{- end }}
    {{- if .context.Values.git.plugins.enabled }}
      {{- range .context.Values.git.plugins.repositories }}
      is_mounted_dir_empty "/plugins_{{ include "airflow.git.repository.name" . }}" && git -c http.sslVerify=false clone {{ .repository }} --branch {{ .branch }} /plugins_{{ include "airflow.git.repository.name" . }}
      {{- end }}
    {{- end }}
{{- end }}
  volumeMounts:
